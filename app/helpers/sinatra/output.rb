...
helpers do
  ...
  def output(resource, data=nil)
    ...
    hash[content_key.urlify] = data unless data == nil

    hash = hash.insert("message", {count: hash[content_key.urlify].count}, :after) if request.request_method == "GET" && data.kind_of?(Array) && data != nil
    ...
    if @ObjectCountForPaging != nil
      ...
      hash = hash.insert("message", {pages: page_hash}, :after)
